...
export class IndexerClient extends BaseClient {
  ...
  constructor({ protocol, hostname, port, pathnamePrefix = `${config.http.prefix}`, secret, agent }:
  ...
  getIndexEvents(indexId: string, indexValue: string, callback: (err: Error, result: GetIndexEventsResult) => void) {
