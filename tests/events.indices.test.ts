  ...
  it(`expects to return 2 events out of 3 for the value '${INDEX_VALUE}'`, (done: Mocha.Done) => {
    ...
    supertest(server)
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.eql({ "id": "blocked-users-by-username", "field": "data.username", "value": INDEX_VALUE, "rows": [EVENTS_ARRAY[0], EVENTS_ARRAY[2]] });
        done();
      });
  });
