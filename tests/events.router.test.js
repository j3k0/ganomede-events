...
const events = require('../src/events.router');
...
describe('events-router', () => {
  ...
  it(`@ ${url}: Valid POST followed by GET on ${channel1}`, (done) => {
    testPost(url, okStat, {
      ...
      testGet(url, okStat, {
        ...
        expect(res.body[0]).to.have.property('id', 0);
  ...
  it(`@ ${url}: 2 Valid POSTs followed by GET on ${channel1}`, (done) => {
    testPost(url, okStat, {
      ...
      testGet(url, okStat, {
        ...
        expect(res.body[0]).to.have.property('id', 0);
        expect(res.body[0]).to.have.property('timestamp');
        expect(res.body[1]).to.have.property('id', 1);
  ...
  it(`@ ${url}: Valid POST followed by GET on ${channel1} after first event`, (done) => {
    let eventId;
    testPost(url, okStat, {
          secret: rightSecret,
          channel: channel1
  ...
  it(`@ ${url}: 2 Valid POSTs followed by GET on ${channel1} after first event`, (done) => {
    let eventId;
    testPost(url, okStat, {
          secret: rightSecret,
          channel: channel1
  ...
  it(`@ ${url}: 3 Valid POSTs followed by GET on ${channel1} after first event`, (done) => {
    let eventId;
    testPost(url, okStat, {
          secret: rightSecret,
          channel: channel1
